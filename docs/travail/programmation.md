...
Au début de nos tests dans la phase « Homing », nous avons été confrontés à deux erreurs de comportement. Le premier souci était que, lors du lancement de la séquence, le moteur semblait parfois ignorer l'interrupteur, continuant sa course bien au-delà de la limite jusqu'à ce que nous devions couper l'alimentation. En examinant notre code, nous avons compris que notre logique conditionnelle dans la boucle while(!XlimitIsPressed()) était trop dépendante d'un signal électrique instable. Si le moteur accélérait trop vite ou si le câblage n'était pas parfaitement fixé, le signal de l'interrupteur passait d'un état à l'autre de façon erratique avant même d'être atteint, ce qui empêchait le programme de valider correctement la détection.
